...
$allow_rename = true; // Set to true to allow rename files
...
if($PASSWORD) {
	...
	if(!$_SESSION['_sfm_allowed']) {
		// sha1, and random bytes to thwart timing attacks.
...
a.download {display:inline-block;
	...
	color:#d00;	margin-left: 15px;font-size:11px;padding:4px 0 4px 20px;
}
a.rename {display:inline-block;
	background: url('data:image/png;base64,') no-repeat scroll 0px 5px;
	color:#d00;	margin-left: 15px;font-size:11px;padding:4px 0 4px 20px;
}
</style>
...
$(function(){
	...
	$('#table').on('click','.rename',function(data) {
		...
		var oldname = $(this).attr('data-file');
		input.id = "name";
		input.type = "text";
		input.name = "name";
		input.value = $(this).attr('data-file');
		$( "a[class='name']:contains("+input.value+")" ).parent()[0].appendChild(input).focus();
		$( "input[id='name']" ).focusout(function() {
			if( $(this).attr('data-file') != $( "input[id='name']" ).val() && $( "input[id='name']" ).val() != '' ){
				$.post("",{'do':'rename','file':oldname,'newname':$( "input[id='name']" ).val(),'xsrf':XSRF},function(response){
					list();
				},'json');
			}
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var $mv_link = $('<a href="#" />').attr('data-file',data.path).addClass('rename').text('rename');
		...
		var $html = $('<tr />')
			...
			.append( $('<td/>').append($dl_link).append($mv_link).append( data.is_deleteable ? $delete_link : '') )
		return $html;
	}
...
<footer>very simple filemanager by <a href="https://github.com/mitagmio">mitagmio</a></footer>
</body></html>
